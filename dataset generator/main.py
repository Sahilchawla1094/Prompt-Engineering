...
uploaded_files = st.file_uploader("Upload your files:", type=['pdf', 'docx', 'txt', 'html', 'csv', 'py'])

if uploaded_files:
    try:
        file_bytes = uploaded_files.read() # BytesIO
        file_name = os.path.join('./files', uploaded_files.name)
        ...
        st.error(f"Failed to read {uploaded_files.name}: {e}")
    else:
